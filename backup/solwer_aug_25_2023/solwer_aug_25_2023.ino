...
void sendData(float temp, float pH, float EC) {
  ...
  while (client.available() == 0) {
    if (millis() - timeout > 1000) //1 second
    {
...
void printData(void) {                            //this function will pars the string  
  static unsigned long timepoint = millis();
    if(millis()-timepoint > 3600) { //time interval: 2.5 minutes
    timepoint = millis();
    ...
    char *EC_str = strtok(sensorstring_array, ",");               //let's pars the array at each comma               
    FLOAT_EC= atof(EC_str);                                 //uncomment this line to convert the char to a float
...
// 5 sec <- delay for sending
// 2.5 min -> fetching of sensors
// 10 sec
// 1 sec <-
// 1 minute <- initializing
