...
boolean phCalibrationEnabled = false;
boolean ecCalibrationEnabled = false;
...
void serialEvent() {
  ...
  if (inputstring == "phcalenter") {
    phCalibrationEnabled = true;
    Serial.println("pH Calibration Enabled");
  } else if (inputstring == "phcalexit") {
    phCalibrationEnabled = false;
    Serial.println("pH Calibration Disabled");
  } else if (inputstring == "eccalenter") {
    ecCalibrationEnabled = true;
    Serial.println("EC Calibration Enabled");
  } else if (inputstring == "eccalexit") {
    ecCalibrationEnabled = false;
    Serial.println("EC Calibration Disabled");
  }
}
...
void loop() {
  ...
  if (sensor_string_complete == true) {
    if (isdigit(sensorstring[0]) == false) {
      ...
      // Perform calibration only if it is enabled
      if (phCalibrationEnabled) {
        ph.calibration(voltage, amb_temperature);
      }
...
void sendData(float temp, float pH, float EC) {
  ...
  client.print(String("GET http://192.168.1.25/solwer/scripts/datalog.php?") +
               ("&temp=") + String(temp) +
               ("&pH=") + String(pH) +
               ("&EC=") + String(EC) +
               ("&GHName=RDGreenhouse") +
